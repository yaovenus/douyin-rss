...
import sys
import json
from urllib.parse import quote  # 必须引入这个用于处理链接
...
def main():
    ...
    if latest_id != last_id:
        ...
        # 1. 构造显示的文本 (点击标题可跳转原网页)
        msg_text = f"📢 <b><a href='{latest_link}'>{latest_title}</a></b>"

        # 2. 构造“使用 CorsaBot 阅读”的按钮
        # 这里的原理是生成一个链接：https://t.me/CorsaBot?start=原始链接
        # 用户点击后，会跳转到 CorsaBot，机器人通常会自动识别并转换
        encoded_link = quote(latest_link)
        keyboard = {
            "inline_keyboard": [
                [
                    {
                        "text": "⚡️ 生成预览 (CorsaBot)", 
